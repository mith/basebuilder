...
fn mouse_input(
    ...
    mut player_query: Query<(&mut Aim, &GlobalTransform, Entity), With<PlayerControlled>>,
    children_query: Query<&Children>,
    mut gun_query: Query<&mut Gun>,
) {
    for (mut aim, player_transform, player_entity) in &mut player_query {
        // calculate angle between cursor and dude
        let angle = angle_between(&cursor_position, player_transform);
        aim.aim_direction = Some(angle);

        for descendant in children_query.iter_descendants(player_entity) {
            if let Ok(gun) = gun_query.get_mut(descendant).as_mut() {
...
fn angle_between(cursor_position: &CursorPosition, dude_transform: &GlobalTransform) -> f32 {
...
mod test {
    ...
    fn angle_between_dude_and_position_up() {
        ...
        let angle = angle_between(&cursor_position, &dude_transform);
